    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color(Id, "cyanBright"), color(Username), color(Country, "green"), color(Region, "redBright"), color(Crowns, "cyanBright"), color(SkillRating, "blueBright")].join(" | ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
